...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function getWeather(cords) {
        showLoaderBackdrop();
        try {
            const response = await fetch(`http://localhost:8081/api/v1/weather?${cords}`);
    ...
    function updateListContent() {
        list = data.list.filter((date) => date.dt_txt.includes(`${mode === "day" ? "09" : "18"}:00:00`));
    }

    btnInputElement.addEventListener("click", () => {
        const cityName = cityInputElement.value;
        if (cityName.trim().length == 0) {
            return getTooltip().show(cityInputElement, "Input field couldn't be empty");
        }
    ...
    geoElement.addEventListener("click", () => {
        ...
        navigator.geolocation.getCurrentPosition(
            ...
            (err) => {
                getTooltip().show(geoElement, "Please enable access to your geolocation");
            }
        );
    });
