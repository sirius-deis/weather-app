...
function updateInfoOnMode(list, n = 0) {
    ...
    dateElement.textContent = `${months[dateTime.getUTCMonth()]} ${dateTime.getDate()}`;
    ...
    centerElement.style.backgroundImage = `url(assets/icons/bg/${list[n].weather[0].icon.replace("n", "d")}.webp)`;
...
function formCards(list) {
    ...
    const elements = list.map((el, i) => {
        ...
        clone.querySelector(".bottom__temperature").textContent = el.main.temp.toFixed(1);
        if (i === 0) {
